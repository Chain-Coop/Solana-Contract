...
// Contract Program ID
const PROGRAM_ID = new PublicKey('948Qgeb7fGxH7tZ8uCdPyEEBw2zm9vk83e57Besee4yb');
...
describe('ChainCoopSaving - Complete Test Suite', () => {
  ...
  before(async function() {
    ...
    const airdropPromises = [
      ...
      connection.requestAirdrop(user1.publicKey, 3 * LAMPORTS_PER_SOL),
      connection.requestAirdrop(user2.publicKey, 3 * LAMPORTS_PER_SOL),
    ...
    await mintTo(connection, owner, mint, user1TokenAccount, owner, BigInt(1000 * 1e9));
    await mintTo(connection, owner, mint, user2TokenAccount, owner, BigInt(1000 * 1e9));
  ...
  describe('1. Contract Deployment Verification', () => {
    ...
    it('Should verify program is owned by BPF Loader', async () => {
      const accountInfo = await connection.getAccountInfo(PROGRAM_ID);
      const bpfLoaderUpgradeable = new PublicKey('BPFLoaderUpgradeab1e11111111111111111111111');
  ...
  describe('2. Initialize Contract', () => {
    it('Should initialize contract with fee recipient', async function() {
      ...
      try {
        const data = InstructionBuilder.encodeInitialize(feeRecipient.publicKey);
        ...
        console.log('⚠️  Initialize test skipped (requires proper ABI encoding)');
    ...
    it('Should fail to initialize twice', async function() {
      this.timeout(30000);
      console.log('⚠️  Test requires successful first initialization');
    });
  });
  ...
  describe('11. Withdraw - LOCK Pool (Early - With Fee)', () => {
    ...
    it('Should transfer fee to fee recipient', async function() {
      ...
      console.log('Fee recipient balance before:', feeBalanceBefore.value.uiAmount);
